...
estaEnTupla :: String -> [(String,String)] -> Bool
estaEnTupla _ [] = False
estaEnTupla n (x:xs) | n == (fst x) || n == (snd x) = True
                     | otherwise = estaEnTupla n xs

sumatoria :: [Int] -> Int -> Int
sumatoria [] n = n
sumatoria xs n = sumatoria (tail xs) (n + (head xs))
...
atajaronSuplentes _ xs n = n - (sumatoria xs 0)
...
equiposValidos ((a,b):xs) | a == b || estaEnTupla a xs || estaEnTupla b xs = False
...
porcentajeDeGoles a xs g = golesAux a xs g g


--          arquero   (equipo, arquero)    goles    total    res        (total no cambia)
golesAux :: String -> [(String,String)] -> [Int] -> [Int] -> Float
golesAux a (x:xs) (g:gs) t | a == (snd x) = division g (sumatoria t 0)
                           | otherwise = golesAux a xs gs t


-- 4

vallaMenosVencida :: [(String,String)] -> [Int] -> String
vallaMenosVencida xs g = menosVencidaAux xs (" ") g (head g)

menosVencidaAux :: [(String,String)] -> String -> [Int] -> Int -> String
menosVencidaAux _ a [] _ = a
menosVencidaAux (x:xs) a (g:gs) men | g <= men = menosVencidaAux xs (snd x) gs g
                                    | otherwise = menosVencidaAux xs a gs men
