...
perteneceT :: (Eq t) => t -> [(t,t)] -> Bool
perteneceT _ [] = False
perteneceT n (x:xs) | n == fst x || n == snd x = True
                    | otherwise = perteneceT n xs
...
porcentajeDeVotosAfirmativos _ v vt = division (sumatoria v) vt
...
formulasInvalidas ((a,b):xs) | a == b = True
                             | perteneceT a xs || perteneceT b xs = True
...
porcentajeDeVotos vice f votos = porcentajeAux vice f votos (sumatoria votos)

porcentajeAux :: String -> [(String, String)] -> [Int] -> Int -> Float
porcentajeAux _ _ [v] t = division v t
porcentajeAux vice ((a,b):xs) (v:vs) t | vice == b = division v t
                                       | otherwise = porcentajeAux vice xs vs t


-- 4 

menosVotado :: [(String, String)] -> [Int] -> String
menosVotado ((a,b):fs) (v:vs) = menosVotadoAux ((a,b):fs) (v:vs) v a

--                formulas             votos    menor  candidato res
menosVotadoAux :: [(String,String)] -> [Int] -> Int -> String -> String
menosVotadoAux _ [] _ c = c
menosVotadoAux ((a,b):fs) (v:vs) m c | v < m = menosVotadoAux fs vs v a
                                     | otherwise = menosVotadoAux fs vs m c
